...
#[test]
fn read_state_script_with_const_value_more_than_once() {
    // Declare some symbols
    let output_y            = FloScriptSymbol::with_name("y");
    ...
    // Get the stream from our state (first time starts it running, second time will clone the output from the first)
    let mut output_stream_1 = executor::spawn(host.notebook().receive_output::<i32>(output_y).expect("output stream"));
    let mut output_stream_2 = executor::spawn(host.notebook().receive_output::<i32>(output_y).expect("output stream"));

    // Should send an update of '3' to the stream
    let next_state_y1       = output_stream_1.wait_stream().expect("at least one update").expect("no errors");
    let next_state_y2       = output_stream_2.wait_stream().expect("at least one update").expect("no errors");
    assert!(next_state_y1 == 3);
    assert!(next_state_y2 == 3);
}
